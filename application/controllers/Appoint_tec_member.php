...
class Appoint_tec_member extends CI_Controller {
	function __Construct()
	{
    parent::__Construct ();
    $this->load->database();
    $this->load->model('Appoint_tec_member_model');
  ...
  {   
    $this->load->view('appoint_tec_member'); 
  }
  ...
  {   
    ...
    $this->db->limit(5);
    $query = $this->db->select(array('nic','name','address','phone','fax','email'))->get('tec_members');
  ...
  public function store(){
    ...
    $insert = $this->Appoint_tec_member_model->create();
    if($insert=='ok'){
