...
class Appoint_pro_member extends CI_Controller {
	function __Construct()
	{
    parent::__Construct ();
    $this->load->database();
    $this->load->model('Appoint_pro_member_model');
  ...
  {   
    $this->load->view('appoint_pro_member'); 
  }
  ...
  {   
    ...
    $this->db->limit(5);
    $query = $this->db->select(array('nic','name','address','phone','fax','email'))->get('procurement_members');
  ...
  public function store(){
    ...
    $insert = $this->Appoint_pro_member_model->create();
    if($insert=='ok'){
